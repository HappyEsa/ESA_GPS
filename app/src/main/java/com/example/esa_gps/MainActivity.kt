...
import com.example.esa_gps.databinding.ActivityMainBinding
import java.util.*
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mainBinding.buttonLocation.setOnClickListener{
            getLocation()
        }
    }
    ...
    private fun getLocation(){
        if(checkPermissions()){
            if(isLocationEnabled()){
                mFusedLocationProviderClient.lastLocation.addOnCompleteListener(this){ task ->
                    ...
                    if(location != null){
                        ...
                        val list:List<Address> =
                            geocoder.getFromLocation(location.latitude,location.longitude,1) //mengambil data wilayah dari latitude dan longitude yang sudah didapatkan

                        mainBinding.apply{//merubah atau berinteraksi dengan layout
                            latitude.text = "Garis Lintang\n${list[0].latitude}"
                            longitude.text = "Garis Bujur\n${list[0].longitude}"
                ...
                val intent = Intent(Settings.ACTION_LOCATION_SOURCE_SETTINGS) //untuk membuka aplikasi setting untuk mengaktifkan layanan lokasi
